...
Z3_ast k1_cnstrs(RSSKS_cfg_t rssks_cfg, Z3_context ctx, Z3_ast d1, Z3_ast d2)
{
    ...
    d1_tcp_src  = RSSKS_extract_pf_from_d(rssks_cfg, ctx, d1, RSSKS_PF_TCP_SRC);
    d1_tcp_dst  = RSSKS_extract_pf_from_d(rssks_cfg, ctx, d1, RSSKS_PF_TCP_DST);

    d2_ipv4_src = RSSKS_extract_pf_from_d(rssks_cfg, ctx, d2, RSSKS_PF_IPV4_SRC);
    d2_ipv4_dst = RSSKS_extract_pf_from_d(rssks_cfg, ctx, d2, RSSKS_PF_IPV4_DST);

    d2_tcp_src  = RSSKS_extract_pf_from_d(rssks_cfg, ctx, d2, RSSKS_PF_TCP_SRC);
    d2_tcp_dst  = RSSKS_extract_pf_from_d(rssks_cfg, ctx, d2, RSSKS_PF_TCP_DST);
...
int validate(RSSKS_cfg_t cfg, RSSKS_key_t k1, RSSKS_key_t k2)
{
    ...
    for (int i = 0; i < 25; i++)
    {
        ...
        printf("\n*** port 1 \n\n");
        RSSKS_print_headers(cfg, h1_1);
        RSSKS_print_hash_output(o1_1);

        RSSKS_print_headers(cfg, h1_2);
        RSSKS_print_hash_output(o1_2);
        ...
        printf("\n*** port 1 (~ port 2)\n\n");
        RSSKS_print_headers(cfg, h12_1);
        RSSKS_print_hash_output(o12_1);

        printf("\n*** port 2 (~ port 1)\n\n");
        RSSKS_print_headers(cfg, h12_2);
        RSSKS_print_hash_output(o12_2);
