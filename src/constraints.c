 ...
 * @example
 */
Z3_ast R3S_cnstr_symmetric_tcp(R3S_cfg_t cfg, unsigned iopt, Z3_ast p1, Z3_ast p2)
{
    R3S_status_t status;
    ...
    status = R3S_packet_extract_pf(cfg, iopt, p1, R3S_PF_TCP_SRC, &p1_tcp_src);
    if (status != R3S_STATUS_SUCCESS) return NULL;

    status = R3S_packet_extract_pf(cfg, iopt, p1, R3S_PF_TCP_DST, &p1_tcp_dst);
    if (status != R3S_STATUS_SUCCESS) return NULL;

    status = R3S_packet_extract_pf(cfg, iopt, p2, R3S_PF_TCP_SRC, &p2_tcp_src);
    if (status != R3S_STATUS_SUCCESS) return NULL;
    
    status = R3S_packet_extract_pf(cfg, iopt, p2, R3S_PF_TCP_DST, &p2_tcp_dst);
    if (status != R3S_STATUS_SUCCESS) return NULL;
 ...
 * @example
 */
Z3_ast R3S_cnstr_symmetric_tcp_ip(R3S_cfg_t cfg, unsigned iopt, Z3_ast p1, Z3_ast p2)
{
    Z3_ast symmetric_ip;
    Z3_ast symmetric_tcp;
    Z3_ast and_args[2];

    symmetric_ip  = R3S_cnstr_symmetric_ip(cfg, iopt, p1, p2);
    if (symmetric_ip == NULL)  return NULL;

    symmetric_tcp = R3S_cnstr_symmetric_tcp(cfg, iopt, p1, p2);
