...
Z3_ast R3S_cnstr_symmetric_tcp(R3S_cfg_t r3s_cfg, unsigned iopt, Z3_context ctx, Z3_ast p1, Z3_ast p2)
{
    ...
    status = R3S_packet_extract_pf(r3s_cfg, iopt, ctx, p1, R3S_PF_TCP_SRC, &p1_tcp_src);
    if (status != R3S_STATUS_SUCCESS) return NULL;

    status = R3S_packet_extract_pf(r3s_cfg, iopt, ctx, p1, R3S_PF_TCP_DST, &p1_tcp_dst);
    if (status != R3S_STATUS_SUCCESS) return NULL;

    status = R3S_packet_extract_pf(r3s_cfg, iopt, ctx, p2, R3S_PF_TCP_SRC, &p2_tcp_src);
    if (status != R3S_STATUS_SUCCESS) return NULL;
    
    status = R3S_packet_extract_pf(r3s_cfg, iopt, ctx, p2, R3S_PF_TCP_DST, &p2_tcp_dst);
    if (status != R3S_STATUS_SUCCESS) return NULL;
...
Z3_ast R3S_cnstr_symmetric_tcp_ip(R3S_cfg_t r3s_cfg, unsigned iopt, Z3_context ctx, Z3_ast p1, Z3_ast p2)
{
    ...
    symmetric_ip  = R3S_cnstr_symmetric_ip(r3s_cfg, iopt, ctx, p1, p2);
    if (symmetric_ip == NULL)  return NULL;

    symmetric_tcp = R3S_cnstr_symmetric_tcp(r3s_cfg, iopt, ctx, p1, p2);
